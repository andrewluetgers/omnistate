...
module.exports = {
	...
	debugCallMatching: {
		operation: function(n, c, eventName, eventNameSpace) {
			//console.log("debug call matching", arguments);
			if (c) {
				console.time('callMatching');
				//console.debug("CHANGE - callMatchingListeners", listeners, n, c, getMatches(listeners, c));
				c && callMatchingListeners(listeners[eventNameSpace], n, c);
	...
	subscribe: function(eventNameSpace, path, cb) {
		...
		var listenersPath = path.join(".") + "." + LKEY,
