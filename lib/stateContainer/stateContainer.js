...
function setDeepWithStableArrayLength(structure, path, value, oldOracle, newOracle) {
	var currentObject = structure,
	    currentOOracle = oldOracle || {},
	    currentNOracle = newOracle || {},
	    val, isArray;
	...
	path.forEach(function(property, index) {
		if (index + 1 === path.length) {
			...
			if (isArray && currentOOracle.length != currentNOracle.length) {
				val.length = currentNOracle.length;
			}

			currentObject[property] = val;
		}
		currentObject = currentObject[property];
		currentOOracle = currentOOracle[property] || {};
		currentNOracle = currentNOracle[property] || {};
	});
